...
|----------|------|-----|-------------|-----|
| `t3.small` | 2 | 2 GB | ~$15 | Too tight — Claude agents use 500MB+ each |
| `t3.medium` | 2 | 4 GB | ~$30 | Only viable with MAX_CONCURRENT capped at 2 |
| **`t3.large`** | **2** | **8 GB** | **~$61** | **Good starting point — comfortably runs 5 concurrent agents** |
| `t3.xlarge` | 4 | 16 GB | ~$121 | Overkill unless agent count goes up significantly |

**Recommendation: Start with `t3.large` ($61/mo).** Each spawned `claude` process uses 500MB+ of RAM, and the dispatcher supports up to 5 concurrent agents (2.5 GB just for agents). Combined with the OS (~500 MB) and Node server (~200 MB), a t3.medium's 4 GB is too tight. The t3.large's 8 GB provides comfortable headroom without needing to artificially cap concurrency.
...
**Budget option (no ALB, Caddy for HTTPS):** ~$69/mo

**Downgrade option (t3.medium, MAX_CONCURRENT=2):** ~$58/mo
...
Each spawned `claude` process can use 500MB+ of RAM. With `MAX_CONCURRENT = 5`, that's potentially 2.5 GB just for agents, plus the OS (~500 MB) and Node server (~200 MB). The t3.large (8 GB) handles this comfortably with ~4.5 GB headroom.

**Mitigation**: Monitor memory usage with CloudWatch. If running more than 5 concurrent agents becomes necessary, upgrade to t3.xlarge.
...
**Decision**: Claude Max subscription. Jess will SSH into the EC2 instance on initial setup and run `claude auth login` interactively to authenticate her Anthropic subscription. The dispatcher's existing behavior of stripping `ANTHROPIC_API_KEY` (to force subscription auth) remains unchanged. If auth expires, re-authenticate manually via SSH.
...
**Decision**: The EC2 instance will have clone access to all project repos configured in Agent Kanban. Repos will be cloned during project setup, and `repoPath` values updated to their EC2 locations. The GitHub App (Phase 0.2) provides the credentials for clone/push access.
...
**Decision**: Single user only (Jess). No multi-user or multi-instance support needed. This confirms the single-process architecture, SQLite over RDS, and no need for distributed locking or user management beyond basic access auth.
