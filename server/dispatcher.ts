...
async function startAgent(ticket: Ticket) {
  ...
  if (ticket.planOnly) {
    ...
    taskLines.push(
      '## Plan-Only Mode',
      '',
      'Do NOT write code. Investigate the codebase and write a report to `plan-report.md` with:',
      'Summary, Relevant Files, Current Architecture, Proposed Approach, Files to Modify, Risks, and Open Questions.',
      '',
      '---',
    );
  } else {
  ...
  if (useWorktree) {
    taskLines.push(
      ...
      `## PR Creation`,
      '',
      `Read \`.github/pull_request_template.md\`, fill in every section (including Ticket-ID: \`${ticket.id}\`),`,
      `and create the PR with \`gh pr create --base ${project.defaultBranch}\` using a HEREDOC body. Do NOT use \`--fill\`.`,
    );
  } else {
    taskLines.push(
